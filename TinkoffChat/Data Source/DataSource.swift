...
class DataSource {
    ...
    class func parseDataSource() -> [ConversationItemsForSection] {
        ...
        var conversationCellItemsDictionarry = [Int: ConversationItemsForSection]()

        let sections = json?["sections"] as? [[String: Any]]
        for item in sections! {
            if let id = item["id"] as? Int,
                let status = item["status"] as? String {
                let instaceStatusSection = ConversationItemsForSection(id: id, status: status)
