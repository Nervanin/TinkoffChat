...
class EditingUserProfileViewController: UIViewController, SetUserProfileImageProtocol {
    
    //var userProfileModelUserProfileModel()
    var user = User()
    ...
    //    @objc func setUpGestureRecognizer() {
    ...
    @objc func saveButtonPressed(_ sender: UIButton) {
        if sender == saveButton {
            ...
            if let text = setUserName.text {
                user.name = text
            }
            if let text = setUserDescription.text {
                user.info = text
            }
            if let image = userImage.image {
               // profile.image = image
            }
            userManager.saveUser(user: user) {[unowned self] (success) in
            }
        }
    ...
    func setupData(_ user: User) {
        ...
        setUserDescription.text = user.info
       // profileImageView.image = user.image
        
        if user.info == "" {
            setUserDescription.text = "placeholderString"
            setUserDescription.textColor = UIColor.lightGray
        }
    }
            
//            self.dataManager.saveProfileData(profile: self.profile, handler: { [weak self]( saved : Bool ) in
